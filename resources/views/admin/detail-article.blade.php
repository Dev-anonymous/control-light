...
@section('body')
    ...
    <div>
        ...
        <div class="main-content">
            ...
            <div class="card ">
                <div class="card-header">
                    <h4>Codes uniques de l'article</h4>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="table-responsive">
                                <table class="table table-condensed table-bordered table-hover" style="width: 100%;">
                                    ...
                                    <tbody>
                                        @foreach ($article->codes()->orderBy('id', 'desc')->get() as $k => $el)
                                            <tr>
                                                <td>{{ $k + 1 }}</td>
                                                <td>{{ $el->code }}</td>
                                                ...
                                                <th>
                                                    @if (in_array(auth()->user()->user_role, ['admin']))
                                                        <button class="btn btn-outline-danger delcode"
                                                            value="{{ $el->id }}">
                                                            <i class="fa fa-trash"></i>
                                                        </button>
                                                    @endif
                ...
                <div class="card-footer d-flex justify-content-end">
                    @if (in_array(auth()->user()->user_role, ['admin']))
                        ...
                        <button class="btn btn-danger" data-toggle="modal" data-target="#mdl-addcode">
                            <i class="fa fa-barcode"></i>
                            Ajouter un code
                        </button>
                    @endif
    ...
    <div class="modal fade" id="mdl-addcode" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static">
        <div class="modal-dialog   modal-sm" role="document">
            <div class="modal-content ">
                <div class="modal-header bg-danger text-white font-weight-bold d-flex justify-content-between">
                    <b>Ajouter un code unique de l'article</b>
                    <span style="cursor: pointer" data-dismiss="modal">
                        <i class="fa fa-times-circle p-2 "></i>
                    </span>
                </div>
                <form id="f-addcode" class="was-validated">
                    <div class="modal-body">
                        <input type="hidden" name="article_id" value="{{ $article->id }}">
                        <div class="form-group">
                            <label for="">Code</label>
                            <input class="form-control" required name="code" placeholder="Code unique de l'article" />
                        </div>
                        <div class="form-group" style="display: none" id="rep"></div>
                    </div>
                    <div class="modal-footer">
    ...
    <div class="modal fade" id="mdl-del2" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static">
        <div class="modal-dialog  " role="document">
            <div class="modal-content ">
                <div class="modal-header bg-danger text-white font-weight-bold d-flex justify-content-between">
                    <b>Suppression code </b>
                    <span style="cursor: pointer" data-dismiss="modal">
                        <i class="fa fa-times-circle p-2 "></i>
                    </span>
                </div>
                <form action="#" id="f-del2">
                    <div class="modal-body">
                        <p>
                            <i class="fa fa-trash-alt"></i>
...
@section('js-code')
            ...
            if (!loaded) {
                try {
                    htmlscanner.render(function(decodedText, decodedResult) {
                        ...
                        var ok = confirm(`Code => ${decodedText}  Voulez-vous ajouter ce code ? `);
                        if (ok) {
                            var d = $('.divscan');
                            d.stop();
                            d.slideUp();
    ...
    <script>
        $(function() {
            ...
            $('#f-addcode').submit(function() {
                event.preventDefault();
                var form = $(this);
                var btn = $(':submit', form).attr('disabled', true);
                btn.find('span').removeClass().addClass('spinner-border spinner-border-sm mr-3');
                rep = $('#rep', form);
                rep.removeClass().stop().slideUp();
                var data = form.serialize();
            ...
            $('.delcode').click(function() {
                var id = this.value;
                var mdl = $('#mdl-del2');
                $('[name=code_id]', mdl).val(id);
                mdl.modal('show');
            });

            $('#f-del2').submit(function() {
                event.preventDefault();
                var form = $(this);
                var btn = $(':submit', form).attr('disabled', true);
                btn.find('span').removeClass().addClass('spinner-border spinner-border-sm mr-3');
